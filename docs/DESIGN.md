...
```yaml
schema_version: 1
timezone: "UTC"             # 基准时区（模板日期填充 / 状态机日期；与 Actions cron 同源）
display_timezone: "Asia/Shanghai"  # 仅 report.md 时间显示，不参与任何计算

fetcher:
  concurrency: 5            # 并发拉取数，默认 5，可配置
  timeout_sec: 20           # 单订阅超时，15-30s 区间
  date_offsets: [0, -1, 1]  # 模板链接日期容错（天），依次尝试、成功即停；[0] = 关闭
...
6. `fetcher.date_offsets` 必须为非空整数列表，元素范围 -7..7 且不可重复。
...
**基准时区固定 UTC**，理由：
- GitHub Actions 的 `schedule.cron` 一律按 UTC 解释，`0 */6 * * *` 的 4 个运行点
  （UTC 00/06/12/18）在 UTC 下固定不漂移；换算北京时间为 08/14/20/次日 02，
  若以北京时间为准，则每天有一次运行踩在午夜边界上。
- UTC 18:00 运行时北京已是次日 02:00，国内站点当天的（D+1）文件通常尚未生成，
  此时 `offset 0`（= 北京"昨天"的文件）恰恰是最新有效文件；
  以北京时间为准反而会先 404 再回退到 -1。
- 订阅源时区是混合的（GitHub 类按 UTC，国内站点多按北京时间），偏差恒小于 1 天，
  由 `date_offsets` 的 ±1 天容错统一覆盖 —— 故基准只需取"稳定"，无需"匹配所有源"。

**展示时区分离**：`display_timezone` 仅用于 report.md 的运行时间显示；
模板填充、状态机日期、冷却判定等全部计算链路只认 `timezone`，两者不混用。

**模板日期容错**（仅对含占位符的模板链接生效，普通链接固定单次拉取）：
- 按 `date_offsets` 顺序依次尝试 `today + offset` 天，成功即停；
  全部候选都失败才计一次失败进入状态机。
- 优先返回首个解析出节点的候选；全部候选均无节点时返回首个拉取成功的候选，
  保持既有 `ok` 语义（拉取成功但 0 节点仍算成功），不改变失败计数口径。
- 每次命中后把该偏移记入 `subscription-state.json` 的 `last_date_offset`，
  下次运行优先重试该偏移，其余按 config 顺序补齐 → 稳定源 1 次命中。
- 失败时保留上次记忆（临时故障不应丢失记忆），仅成功才更新。
- 属 fetcher 层容错，与节点质量无关，不引入打分/权重。
...
8. 基准时区 UTC（见 §4.4）；report.md 展示时区由 `display_timezone` 单独配置。
9. 模板链接日期容错：按 `date_offsets` 依次尝试日期偏移，成功即停，全部失败才计失败；
   命中偏移记入 state 并优先重试（仅模板链接，见 §4.4）。
...
- 基准时区改 UTC（与 Actions cron 同源、运行点不漂移）；源时区偏差恒小于 1 天，由 `date_offsets` ±1 天容错覆盖。
- 展示时区与计算时区分离：`display_timezone` 仅影响 report.md 显示，计算链路只认 `timezone`。
- 模板链接日期容错 + 命中偏移记忆（fetcher 层容错，非节点评分，不违反无打分铁律）。

---

## 13. 本地 Docker 模式（可选部署）

云模式（GitHub Actions）之外的自建形态，两种模式共用同一套 pipeline 与 config。

### 13.1 定位

| | 云模式（Actions） | 本地模式（Docker） |
|---|---|---|
| 运行位置 | GitHub 云端 | 自建容器 |
| 数据 | 快照提交进 git | **仍以 GitHub 为准**，本地不回写 |
| 订阅方式 | 公网 raw 链接 | 容器暴露 HTTP，局域网订阅 |

### 13.2 运行形态

```
/app            运行时工作区（git 浅克隆：代码与数据始终与 GitHub 一致）
/opt/sub-sift   镜像自带副本（web/sync 工具 + 远端不可达时的降级来源）
...
- 同步线程：检测远端数据快照 → 丢弃本地 `data/` `output/` 改动 → pull → 运行 `main.py`
- web 服务（FastAPI）：订阅分发 + 主清单/聚合源维护

### 13.3 同步机制（双触发）

| 触发 | 时机 | 行为 |
|---|---|---|
| A 定时 | 上次快照提交时间 + `SYNC_INTERVAL_HOURS` + 缓冲 | 进入检测窗口 |
| B 编辑 | web 编辑推送成功后 | **立即**进入检测窗口 |

窗口内每 `SYNC_POLL_SECONDS` 比对本地与远端的最新 `chore(data)` 提交，
发现新快照即同步。空闲时段零请求，且完全不调用 GitHub API（避开匿名限流）。

**间隔来源**：`interval_hours` 为 0（默认）时，从 `.github/workflows/ci.yml` 的
`schedule.cron` 自动解析 —— cron 才是事实源，容器只跟随、不反向修改它
（改 workflow 需要更高的 `Workflows: Read and write` 权限，不该交给 web 端）。
支持 `*/N`、`0,6,12,18`、单个小时等写法；解析失败回退 6 小时。
填具体数字则手动指定，页面会显示当前来源（自动/手动/回退）。

下次检测时间 = 上次 Actions 快照提交时间 + 生效间隔 + `buffer_minutes`。
`buffer_minutes` 默认 5（Actions 单次运行实测 2-3 分钟），需略大于该耗时：
过短会在 Actions 提交前空跑一轮，过长则无谓延后同步。

四个参数均可在 web 页面「同步参数」中调整，写入 `/app/data/.sync-config.json`
（卷内持久、git 未跟踪，不会被同步覆盖），保存后立即按新参数重算窗口。
「立即检测同步」按钮等价于触发 B，跳过等待直接进入检测窗口。

### 13.4 编辑流程

编辑必须回推 GitHub，否则下次同步会被覆盖（数据以服务器为准）：

```
1. git checkout -- data output    丢弃本地运行产生的统计改动
2. 应用编辑（只改 link / sources 两列，程序维护列保持原值）
3. commit + push（PAT：fine-grained，仅 contents:write + actions:write）
4. 本地运行 main.py              编辑立即在本地 output 生效
5. 请求同步                      立即进入检测窗口，等 Actions 产出快照
```

push 会触发 Actions（`ci.yml` 未加 paths 过滤），编辑因此在服务器上即时生效；
`EDIT_TRIGGER_ACTIONS=false` 时提交加 `[skip ci]`，不惊动 Actions。

### 13.5 端点

| 端点 | 鉴权 | 说明 |
|---|---|---|
| `GET /sub/{plain\|v2ray\|clash}` | 否 | 订阅文件，供局域网设备订阅 |
| `GET /report` | 否 | report.md |
| `GET /api/status` | 是 | 同步状态、快照、下次窗口 |
| `POST /api/sync` | 是 | 立即同步 |
| `/api/subscriptions/*`、`/api/aggregators/*` | 是 | 主清单/聚合源增删改 |

鉴权：`WEB_TOKEN` 登录后写 HttpOnly Cookie（30 天），cookie 内只存 token 的 sha256。
未设置 `WEB_TOKEN` 时管理端免鉴权（仅限可信局域网）。

### 13.6 镜像

`.github/workflows/docker.yml` 构建 `linux/amd64,linux/arm64` 推 GHCR；
仅在 `docker/`、`requirements.txt` 等变化时触发（数据快照带 `[skip ci]`，不触发）。
