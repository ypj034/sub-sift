...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=config.concurrency) as pool:
        futures = {
            pool.submit(
                ...
                config,
                today,
                sub_states[row.link].last_date_offset,
            ): row.link
            for row in plan
        }
        for future in concurrent.futures.as_completed(futures):
            link = futures[future]
            ok, nodes, used_offset = future.result()
            ...
            # 记忆命中的日期偏移（仅模板链接有值）；失败时保留上次记忆，避免临时故障丢失
            if ok and used_offset is not None:
                state.last_date_offset = used_offset
    ...
    # 报告时间按展示时区呈现（计算链路始终用 config.timezone，此处仅转换显示）
    display_now = now.astimezone(ZoneInfo(config.display_timezone))
    report_path = generate_report(
        config,
        {
            "run_time": f"{display_now:%Y-%m-%d %H:%M:%S} {config.display_timezone}",
            "today": today,
            "sub_rows": sub_rows,
    ...
    _print_offset_summary(per_link)
    return 0
...
def _print_offset_summary(per_link: dict[str, dict]) -> None:
    """打印模板链接的日期偏移命中分布（仅运行日志，不进 report.md）。"""
    hits: dict[int, int] = {}
    for info in per_link.values():
        offset = info.get("offset")
        if offset is not None and info.get("ok"):
            hits[offset] = hits.get(offset, 0) + 1
    if hits:
        dist = "，".join(f"{off:+d}天×{hits[off]}" for off in sorted(hits))
        print(f"[sub-sift] 模板日期命中分布: {dist}")
