...
def test_timezone_default_is_utc(tmp_path):
    cfg = load_config(_write(tmp_path, VALID.replace('timezone: "Asia/Shanghai"\n', "")))
    assert cfg.timezone == "UTC"
    assert cfg.display_timezone == "Asia/Shanghai"
    assert cfg.date_offsets == [0]


def _with_date_offsets(expr: str) -> str:
    """在 fetcher 段注入 date_offsets（VALID 经 dedent，缩进为 2 空格）。"""
    return VALID.replace("  timeout_sec: 20", f"  timeout_sec: 20\n  date_offsets: {expr}")


def test_date_offsets_valid(tmp_path):
    cfg = load_config(_write(tmp_path, _with_date_offsets("[0, -1, 1]")))
    assert cfg.date_offsets == [0, -1, 1]


def test_date_offsets_out_of_range(tmp_path):
    with pytest.raises(ConfigError, match="date_offsets"):
        load_config(_write(tmp_path, _with_date_offsets("[0, 30]")))


def test_date_offsets_duplicate(tmp_path):
    with pytest.raises(ConfigError, match="date_offsets"):
        load_config(_write(tmp_path, _with_date_offsets("[-1, -1]")))


def test_date_offsets_non_integer(tmp_path):
    with pytest.raises(ConfigError, match="date_offsets"):
        load_config(_write(tmp_path, _with_date_offsets('[0, "x"]')))


def test_display_timezone_empty(tmp_path):
    bad = VALID + 'display_timezone: ""\n'
