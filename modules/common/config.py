...
def _validate(raw: dict) -> Config:
    ...
    display_timezone = raw.get("display_timezone", "Asia/Shanghai")
    if not isinstance(display_timezone, str) or not display_timezone:
        err("display_timezone 必须是非空字符串（仅影响 report.md 时间显示）")
    ...
    date_offsets = _int_list(
        fetcher.get("date_offsets", [0]),
        "fetcher.date_offsets",
        err,
        lo=DATE_OFFSET_RANGE[0],
        hi=DATE_OFFSET_RANGE[1],
        unique=True,
    )
...
def _int_list(
    value: Any, key: str, err, lo: int, hi: int, unique: bool = False
) -> list[int]:
    """校验整数列表；元素允许为数字字符串（环境变量覆盖后为 str）。"""
    ...
    for item in value:
        try:
            ...
            err(f"{key} 元素必须是整数，当前: {item!r}")
            return [0]
        if not (lo <= iv <= hi):
            err(f"{key} 元素必须在 {lo}..{hi} 之间，当前: {iv}")
            return [0]
        if unique and iv in out:
            err(f"{key} 不允许重复元素: {iv}")
            return [0]
        out.append(iv)
    return out
